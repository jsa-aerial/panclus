...
(def spsbase "/store/data/SPStrains")
...
(def ref-len-dists-map
  (->>  "/store/data/SPStrains/Entropy/len-recs-dists-map.clj"
        slurp read-string))
(def maxNCdists
  (->> "/store/data/SPStrains/Entropy/maxNCdists.clj"
       slurp read-string))
...
(defn cluster-strains
  [dists distmap maxNCdists len-recs-map
...
(defn cluster-leftovers
  [distmap dists all-matched len-recs-map
   & {:keys [len-max %-max jsdctpt] :or {len-max 5 %-max 0.99 jsdctpt 0.40}}]
  (let [distmap (reduce (fn[M k] (dissoc M k)) distmap all-matched)
        len-recs-map (reduce (fn[S [n cnt P :as rec]]
...
(def ref-clusters-040
  (time (cluster-strains refdists refdists-map
                         maxNCdists len-recs-map :jsdctpt 0.40)))
(def all-matched-040
  (->> ref-clusters-040 (mapcat (fn[clu](->> clu butlast (map first))))))
(io/with-out-writer
  "/store/data/SPStrains/Entropy/ref-clusters-040.clj"
...
(def ref-clu-map040
  (reduce (fn[M clu]
          ...
          {} ref-clusters-040))
(io/with-out-writer
  "/store/data/SPStrains/Entropy/ref-clu-map-040"
  (prn ref-clu-map040))


(def ref-clus-leftovers-040
  (time (cluster-leftovers refdists-map refdists all-matched-040 len-recs-map)))
...
(io/with-out-writer
  "/store/data/SPStrains/Entropy/ref-clu-leftover-map-040"
  (prn ref-clus-leftover-map040))

(def total-ref-clus-map040
  (merge ref-clu-map040 ref-clus-leftover-map040))
(io/with-out-writer
  "/store/data/SPStrains/Entropy/total-ref-clus-map-040"
...
(def ref-clu-setmap040
  (->> ref-clusters-040
       ...
       (into {})))
(io/with-out-writer
  "/store/data/SPStrains/Entropy/ref-clu-setmap-040"
...
(def cluscores
  (future (->> ref-clusters-040 (coll/drop-until #(> (count %) 1))
               (mapv aln-score-clu))))
(io/with-out-writer
  "/store/data/SPStrains/Entropy/aln-scores.clj"
  (prn @cluscores))
...
(cluster-strains refdists refdists-map (take 5 (coll/dropv 1000 maxNCdists)))
(cluster-strains refdists refdists-map (take 5 (coll/dropv 2000 maxNCdists)))


(def size-dist
  (sort-by
   first
   (reduce (fn[S [n cnt P]] (assoc S cnt (inc (get S cnt 0)))) {} refdists)))

(p/mean (map second refdists))
281.6591802747837
(p/median (map second refdists))
238.0
(p/std-deviation (map second refdists))
221.6938718239762
(- 282 222)
60
(+ 282 222)
...
(cluster-strains refdists refdists-map [hybdist])
